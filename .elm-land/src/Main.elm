...
initPageAndLayout model =
    case Route.Path.fromUrl model.url of
        ...
        Route.Path.Finished ->
            let
                page : Page.Page Pages.Finished.Model Pages.Finished.Msg
                page =
                    Pages.Finished.page model.shared (Route.fromUrl () model.url)
            ...
            { page = 
                Tuple.mapBoth
                    Main.Pages.Model.Finished
                    (Effect.map Main.Pages.Msg.Finished >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }
        ...
        Route.Path.Play ->
            let
                page : Page.Page Pages.Play.Model Pages.Play.Msg
                page =
                    Pages.Play.page model.shared (Route.fromUrl () model.url)
            ...
            { page = 
                Tuple.mapBoth
                    Main.Pages.Model.Play
                    (Effect.map Main.Pages.Msg.Play >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }
        ...
        Route.Path.Sort ->
            let
                page : Page.Page Pages.Sort.Model Pages.Sort.Msg
                page =
                    Pages.Sort.page model.shared (Route.fromUrl () model.url)
            ...
            { page = 
                Tuple.mapBoth
                    Main.Pages.Model.Sort
                    (Effect.map Main.Pages.Msg.Sort >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }
...
updateFromPage msg model =
    case ( msg, model.page ) of
        ...
        ( Main.Pages.Msg.Finished pageMsg, Main.Pages.Model.Finished pageModel ) ->
            Tuple.mapBoth
                Main.Pages.Model.Finished
                (Effect.map Main.Pages.Msg.Finished >> fromPageEffect model)
                (Page.update (Pages.Finished.page model.shared (Route.fromUrl () model.url)) pageMsg pageModel)

        ( Main.Pages.Msg.Play pageMsg, Main.Pages.Model.Play pageModel ) ->
            Tuple.mapBoth
                Main.Pages.Model.Play
                (Effect.map Main.Pages.Msg.Play >> fromPageEffect model)
                (Page.update (Pages.Play.page model.shared (Route.fromUrl () model.url)) pageMsg pageModel)

        ( Main.Pages.Msg.Sort pageMsg, Main.Pages.Model.Sort pageModel ) ->
            Tuple.mapBoth
                Main.Pages.Model.Sort
                (Effect.map Main.Pages.Msg.Sort >> fromPageEffect model)
                (Page.update (Pages.Sort.page model.shared (Route.fromUrl () model.url)) pageMsg pageModel)
...
toLayoutFromPage model =
    case model.page of
        ...
        Main.Pages.Model.Finished pageModel ->
            Route.fromUrl () model.url
                |> Pages.Finished.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Finished >> Page))

        Main.Pages.Model.Play pageModel ->
            Route.fromUrl () model.url
                |> Pages.Play.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Play >> Page))

        Main.Pages.Model.Sort pageModel ->
            Route.fromUrl () model.url
                |> Pages.Sort.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Sort >> Page))
    ...
    let
        ...
        subscriptionsFromPage =
            case model.page of
                ...
                Main.Pages.Model.Finished pageModel ->
                    Page.subscriptions (Pages.Finished.page model.shared (Route.fromUrl () model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Finished
                        |> Sub.map Page

                Main.Pages.Model.Play pageModel ->
                    Page.subscriptions (Pages.Play.page model.shared (Route.fromUrl () model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Play
                        |> Sub.map Page

                Main.Pages.Model.Sort pageModel ->
                    Page.subscriptions (Pages.Sort.page model.shared (Route.fromUrl () model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Sort
                        |> Sub.map Page
...
viewPage model =
    case model.page of
        ...
        Main.Pages.Model.Finished pageModel ->
            Page.view (Pages.Finished.page model.shared (Route.fromUrl () model.url)) pageModel
                |> View.map Main.Pages.Msg.Finished
                |> View.map Page

        Main.Pages.Model.Play pageModel ->
            Page.view (Pages.Play.page model.shared (Route.fromUrl () model.url)) pageModel
                |> View.map Main.Pages.Msg.Play
                |> View.map Page

        Main.Pages.Model.Sort pageModel ->
            Page.view (Pages.Sort.page model.shared (Route.fromUrl () model.url)) pageModel
                |> View.map Main.Pages.Msg.Sort
                |> View.map Page
...
toPageUrlHookCmd model routes =
    ...
    case model.page of
        ...
        Main.Pages.Model.Finished pageModel ->
            Page.toUrlMessages routes (Pages.Finished.page model.shared (Route.fromUrl () model.url)) 
                |> List.map Main.Pages.Msg.Finished
                |> List.map Page
                |> toCommands

        Main.Pages.Model.Play pageModel ->
            Page.toUrlMessages routes (Pages.Play.page model.shared (Route.fromUrl () model.url)) 
                |> List.map Main.Pages.Msg.Play
                |> List.map Page
                |> toCommands

        Main.Pages.Model.Sort pageModel ->
            Page.toUrlMessages routes (Pages.Sort.page model.shared (Route.fromUrl () model.url)) 
                |> List.map Main.Pages.Msg.Sort
                |> List.map Page
                |> toCommands
